<template>
  <div>
    <v-text-field v-model="filter" prepend-icon="search" type="number" label="Phone.." solo></v-text-field>
    <v-card class="fnt">
      <!-- <p>{{filter}}</p> -->
      <!-- <p>{{testFun}}</p> -->
      <v-layout row wrap>
      ...
      >
        <v-flex xs3 md2 class="pa-3" @click="getLi(x)">
...
export default {
  ...
  methods: {
    ...
    getEdit(x) {
      this.$router.push(`/patient/updatePatient/${x._id}`);
    },
    //For Delete This User
    getDelete(x, i) {
      this.$http
        .delete(`/patient/${x._id}`)
        .then(res => {
          console.log(res);
        })
        .then(() => {
